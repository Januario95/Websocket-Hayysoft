...
def Get_Mqtt_Data(data_from_gateway):
    ...
    for row in data_from_gateway:
        ...
        if row['type'] == 'Unknown':
            ...
            print(json.dumps(mqtt_data_, indent=4))
            # MQTT_DATA = mqtt_data_

            query = '''
                INSERT INTO TBL_Mqtt (Mqtt_ID, Timestamp, Type, Mac,
                                      BleName, Rssi, RawData)
                VALUES ((SELECT CREATE_PK('MQTT')), CURRENT_TIMESTAMP(),
                        %s, %s, %s, %s, %s);
            '''
            parameters = (Type, Device_Mac, BleName, Rssi, RawData)
            Cursor.execute(query, parameters)
            Connector.commit()
            print('*' * 10, 'Data Saved Successfully', '*' * 10)
